...
return <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/10 shadow-metal py-1 h-[80px]">
      <div className="container mx-auto px-2 lg:px-4">
        <div className="flex items-center w-full justify-between gap-2 lg:gap-4 xl:gap-8">
          {/* Logo/Branding - Left */}
          <Link to="/" className="flex items-center flex-shrink-0 group">
            <img src={tissLogo} alt="TISS - Travel in Supreme Style" className="h-[72px] lg:h-[76px] w-auto object-contain" />
          </Link>

          {/* Desktop Navigation - Center */}
          <div className="hidden xl:flex items-center justify-center flex-1 gap-3 2xl:gap-5">
            {navLinks.map(link => <Link key={link.path} to={link.path} className={`text-sm font-medium transition-colors hover:text-primary px-2 whitespace-nowrap ${isActive(link.path) ? "text-primary" : "text-white/60 hover:text-white"}`}>
                {link.label}
              </Link>)}
          </div>
